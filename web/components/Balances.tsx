...
// Balances shows the user's per-asset main/locked balances, with a dev faucet
// for funding the demo before a custody integration exists.
    ...
    <Panel title="Balances">
      ...
      {balances.map((b, i) => (
        <div key={i} className="grid grid-cols-3 px-2 py-[3px] num text-xs">
          ...
          <span className="text-right text-gray-200">{fmt(b.balance)}</span>
        </div>
      ))}
      {loggedIn && (
